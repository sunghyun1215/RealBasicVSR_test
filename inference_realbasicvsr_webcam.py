...
def main():
    ...
    if file_extension in VIDEO_EXTENSIONS:  # input is a video file
        ...
        for input_path in input_paths:
            img = mmcv.imread(input_path, channel_order='rgb') # (h, w, c)
            inputs.append(img)
    else:
        raise ValueError('"input_dir" can only be a video or a directory.')

    for i, img in enumerate(inputs):
        img = torch.from_numpy(img / 255.).permute(2, 0, 1).float() # (c, h, w)
        inputs[i] = img.unsqueeze(0) # (1, c, h, w)
    inputs = torch.stack(inputs, dim=1) # (1, # of images, c, h, w)
    ...
    with torch.no_grad():
        if isinstance(args.max_seq_len, int):
            ...
            for i in range(2, inputs.size(1), args.max_seq_len):
            # for i in range(0, inputs.size(1)):
                # imgs = inputs[:, i, :, :, :]
                # imgs = inputs[:, i:i+args.max_seq_len, :, :, :]
                imgs = inputs[:, i-args.max_seq_len:i, :, :, :]
                if cuda_flag:
                    imgs = imgs.cuda()
